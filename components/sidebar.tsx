...
export default function Sidebar({ open, setOpen }: { open: boolean; setOpen: (open: boolean) => void }) {
  return (
    <aside
      className={`${open ? "w-64" : "w-20"} transition-all duration-300 bg-white border-r border-border h-screen overflow-y-auto sticky top-0`}
    >
      ...
      <nav className="p-4 space-y-2">
        ...
        {/* Products Section */}
        {open && <div className="mt-6 mb-3 px-3 text-xs font-semibold text-muted-foreground uppercase">Products</div>}
